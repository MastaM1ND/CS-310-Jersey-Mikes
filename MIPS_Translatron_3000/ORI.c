...
void ori_immd_assm(void) {
    ...
    */
    // Rt (destination) should be 31 or less
    if (PARAM1.value > 31) {
        state = INVALID_REG;
        return;
    }

    // Rs (source) should be 31 or less
    if (PARAM2.value > 31) {
        state = INVALID_REG;
        return;
    }
    ...
    /*
        Putting the binary together for ORI (I-type):
        Opcode (bits 31..26) = 001101
...
void ori_immd_bin(void) {
    // Check if the opcode bits match for ORI
    ...
    */
    ...
    /*
        ...
        For ORI, the assembly syntax is "ORI rt, rs, immediate"
        where rt is destination, rs is source, and immediate is the 16-bit constant.
    */
    setOp("ORI");
